...
export const makeMessagesSocket = (config) => {
    ...
    const getUSyncDevices = async (jids, useCache, ignoreZeroDevices) => {
        ...
        const toFetch = [];
        const jidsWithUser = [];
        for (let i = 0, len = jids.length; i < len; i++) {
            let jid = jids[i];
            ...
            if (typeof device === 'number' && device >= 0 && user) {
                ...
                jidsWithUser.push({ jid: jidNormalizedUser(jid), user });
            }
        }
        let mgetDevices;
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, addBizAttributes, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            // melody@Changes 02-02-26 --- Normalize message first to extract the original message and valid media type
            ...
            if (isNewsletter) {
                ...
                // melody@Changes 08-02-26 --- Add "additionalNodes" for newsletter too (⁠っ⁠˘̩⁠╭⁠╮⁠˘̩⁠)⁠っ
                ...
                binaryNodeContent.push({
                    tag: 'plaintext',
                    attrs: extraAttrs, // melody@Changes 02-02-26 --- Add extraAttrs to fix media being rejected when sending to newsletter (⁠◠⁠‿⁠◕⁠)
                    content: bytes
                });
                const stanza = {
                    tag: 'message',
            ...
            // melody@Changes 02-02-26 --- Add keepInChat, editedMessage, mediaNotifyMessage and pollUpdateMessage
            ...
            // melody@Changes 02-02-26 --- Add native_flow_name to extraAttrs when sending interactiveResponseMessage
            ...
            if (isGroupOrStatus && !isRetryResend) {
                ...
                for (let i = 0, len = devices.length; i < len; i++) {
                    const { user, jid } = devices[i];
                    if (jid === meId || (meLid && jid === meLid)) continue;
                    
                    if (user === mePnUser || user === meLidUser) {
            ...
            // melody@Changes 30-01-26 --- Add Biz Binary Node to support button messages
    ...
    const getMediaType = (message) => {
        ...
        if ('videoMessage' in message) return message.videoMessage.gifPlayback ? 'gif' : 'video';
        if ('stickerMessage' in message) return message.stickerMessage.isLottie ? '1p_sticker' : message.stickerMessage.isAvatar ? 'avatar_sticker' : 'sticker';
        if ('audioMessage' in message) return message.audioMessage.ptt ? 'ptt' : 'audio';
        if ('albumMessage' in message) return 'collection';
        if ('contactMessage' in message) return 'vcard';
        if ('documentMessage' in message) return 'document';
        if ('contactsArrayMessage' in message) return 'contact_array';
        if ('liveLocationMessage' in message) return 'livelocation';
        if ('stickerPackMessage' in message) return 'sticker_pack';
        if ('listMessage' in message) return 'list';
        if ('listResponseMessage' in message) return 'list_response';
        if ('buttonsResponseMessage' in message) return 'buttons_response';
        if ('orderMessage' in message) return 'order';
        if ('productMessage' in message) return 'product';
        if ('interactiveResponseMessage' in message) return 'native_flow_response';
        if (message.extendedTextMessage?.matchedText || 'groupInviteMessage' in message) return 'url';
        // melody@Note 02-02-26 --- Add more message type here
        const textContent = message.extendedTextMessage?.text || message.conversation || '';
        if (textContent.includes('://wa.me/c/')) return 'cataloglink';
        if (textContent.includes('://wa.me/p/')) return 'productlink';
        return '';
    };
    ...
    return {
        ...
        // melody@Changes 30-01-26 --- Add support for modifying additionalNodes and additionalAttributes using "options" in sendMessage()
        sendMessage: async (jid, content, options = {}) => {
            const userJid = authState.creds.me.id;
            // melody@Changes 13-03-26 --- Add status mentions!
            if (Array.isArray(jid)) {
                ...
                if (isDeleteMsg || isKeepMsg) {
                    ...
                    additionalNodes.push({
                        tag: 'meta',
                        attrs: {
                            // melody@Note 08-02-26 --- Still a hypothesis regarding PollResult ༎ຶ⁠‿⁠༎ຶ
                ...
                // melody@Changes 30-01-26 --- Add support for AI label in message when "ai" is true, but works only in private chat
                else if (isAiMsg) {
                ...
                // melody@Changes 31-01-26 --- Add support for album messages
                // melody@Note 06-02-26 --- Refactored to reduce high RSS usage (⁠╥⁠﹏⁠╥⁠)
                if ('album' in content) {
                    const { delayMs = 1500 } = options;
